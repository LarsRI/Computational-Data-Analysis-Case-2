...
# Load archetypes
load("archetypes3.RData")
aa7 <- aa3
x <- aa7$alphas
colnames(x) <- paste("Archetype", 1:3)
...
aTypes <- paste("Archetype", 1:3)

pD$Var1 <- factor(rep(aTypes, 3), levels = aTypes[col.ord])
...
setnames(tmp, paste("Archetype", 1:3))
